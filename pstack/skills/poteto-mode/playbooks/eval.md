...
5. **Spawn one blinded judge** using your configured **eval blinded judge** model (same seat as arena cross-judge; default `claude-fable-5-medium-thinking`; fallback `claude-opus-4-8-thinking-high`) per the **arena** skill's Phase C. It must be a different model family from the candidates. Judge sees outputs by sanitized label and the rubric, never a model name.
